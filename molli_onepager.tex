...
\usepackage[margin=0.35in,top=0.3in,bottom=0.35in]{geometry}
...
% ── Equation box ──
\newcommand{\eqbox}[2]{%
  \vspace{0pt}\begin{center}\colorbox{tint}{\parbox{0.87\linewidth}{\vspace{0pt}\begin{equation}#1\tag{#2}\end{equation}\vspace{-4pt}}}\end{center}\vspace{-2pt}}
...
\begin{tikzpicture}[
  ...
  \draw[arr,rounded corners=3pt](f.south)--++(0,-0.2)-|node[below,pos=0.5,lbl]{next domain}(s.south);
...
\textbf{Gene scoring.}
After fine-tuning, we estimate the effect of reverting each gene $i$ on the loss $\mathcal{L}_j$ of prior domain $j$. The gradient of the loss tells us how sensitive each parameter is to small changes; the complement--primary difference tells us how much each parameter actually changed. Their dot product gives the estimated reversion effect:

\eqbox{\delta_{ij} \;=\; \bigl\langle\, \nabla_\theta \mathcal{L}_j,\;\; \mathbf{w}^c_i - \mathbf{w}^p_i \,\bigr\rangle}{1}

This is a first-order Taylor approximation. A single forward+backward pass yields $\nabla_\theta \mathcal{L}_j$ for the entire model, from which all $L\!=\!928$ gene scores are extracted by slicing. Total cost: $\mathcal{O}(N_\text{obj})$ passes, not $\mathcal{O}(L \times N_\text{obj})$.
...
\eqbox{\tau^2 = \max\!\bigl(0,\;\textrm{Var}(\delta) - \sigma^2\bigr), \quad B = \frac{\tau^2}{\tau^2 + \sigma^2}, \quad \delta^*_{ij} = B\,\delta_{ij} + (1\!-\!B)\,\mu_0}{2}
...
\eqbox{P(\textrm{del}_{ij}) = 1 - \Phi\!\!\left(\frac{-\delta^*_{ij}}{\sqrt{\textrm{Var}_\textrm{post}}}\right)\!, \quad \textrm{Repair if}\; \max_j P(\textrm{del}_{ij}) - \alpha\, P(\textrm{ben}_i) > \theta}{3}
...
\textbf{Key findings.}
(i)~MOLLI achieves 23--24\% lower GM PPL at 3 and 5 domains.
(ii)~At 5 domains, every prior domain stays $\leq$1.25; LoRA degrades to 2.29 (medical) and 1.83 (legal).
(iii)~At 6 domains the gap narrows to 5\%, suggesting the fixed 15\% repair cap becomes a bottleneck with 7 simultaneous objectives; an adaptive cap would likely restore the wider gap.
(iv)~139/928 genes repaired per cycle (15\% cap), 0--151 fixed---targeted, selective intervention.
